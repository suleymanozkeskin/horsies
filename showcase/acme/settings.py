# showcase/acme/settings.py
"""Database URL resolution for the Acme Clothing showcase.

Acme Clothing owns its own database (``acme_demo`` by default) so a demo run
never shares rows with the horsies development database. Both the horsies
tables and the ``acme_*`` tables live in that one database.

Resolution order, first match wins:

1. ``ACME_DATABASE_URL`` in the environment — used verbatim.
2. ``ACME_DATABASE_URL`` in the repository ``.env`` — used verbatim.
3. ``DATABASE_URL`` in the environment, with its database name replaced by
   ``acme_demo`` — reuses the local credentials already configured for
...
ACME_URL_VARIABLE: Final[str] = 'ACME_DATABASE_URL'
SHARED_URL_VARIABLE: Final[str] = 'DATABASE_URL'
ACME_DATABASE_NAME: Final[str] = 'acme_demo'
...
DEFAULT_DATABASE_URL: Final[str] = (
    f'{SQLALCHEMY_SCHEME}://postgres:postgres@localhost:5432/{ACME_DATABASE_NAME}'
)
...
class DatabaseSettings:
    """Every connection string Acme Clothing needs, derived from one URL."""
...
def _read_env_file(path: Path) -> dict[str, str]:
    ...
    try:
        ...
        raise SettingsError(
            ...
            f'Set {ACME_URL_VARIABLE} in the environment instead.',
        ) from error
...
def _select_url(env_file: dict[str, str]) -> tuple[str, str]:
    ...
    direct_environment = os.environ.get(ACME_URL_VARIABLE, '').strip()
    if direct_environment:
        return direct_environment, f'environment {ACME_URL_VARIABLE}'

    direct_env_file = env_file.get(ACME_URL_VARIABLE, '').strip()
    if direct_env_file:
        return direct_env_file, f'.env {ACME_URL_VARIABLE}'

    shared_environment = os.environ.get(SHARED_URL_VARIABLE, '').strip()
    if shared_environment:
        return (
            _with_database_name(shared_environment, ACME_DATABASE_NAME),
            f'environment {SHARED_URL_VARIABLE} (database -> {ACME_DATABASE_NAME})',
        )

    shared_env_file = env_file.get(SHARED_URL_VARIABLE, '').strip()
    if shared_env_file:
        return (
            _with_database_name(shared_env_file, ACME_DATABASE_NAME),
            f'.env {SHARED_URL_VARIABLE} (database -> {ACME_DATABASE_NAME})',
        )
