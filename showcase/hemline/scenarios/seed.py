# showcase/hemline/scenarios/seed.py
"""Create the demo database, the `hemline_*` tables, and the catalog.
...
def run() -> int:
    ...
    match store.ensure_schema():
        case Err(error):
            say(f'cannot create the hemline tables: {error.message}')
            return 1
        case Ok(_):
            say('hemline_* tables ready')
    ...
    bullet('  uv run horsies worker showcase.hemline.app:app --processes 12')
    bullet('  uv run horsies scheduler showcase.hemline.app:app')
    bullet('  uv run horsies web showcase.hemline.app:app --enable-actions')
    bullet('  uv run python -m showcase.hemline.scenarios steady')
