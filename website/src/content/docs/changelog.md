...
- Workflow retention batches by workflow, not by node row, in a single
  statement. Candidates come from the workflows retention index; each is
  guarded once per statement against a materialized set of workflows with
  non-terminal backing tasks, computed from the non-terminal side (served
  by the tasks status index) instead of a per-candidate `NOT terminal`
  heap walk. Node rows purge set-wise in the same statement; a workflow
  with more nodes than the whole batch budget drains alone instead of
  starving. Deletion semantics unchanged.
- Finalize logs scope their claim to the finalize-retry loop; the
  broken-pool message states that the task was already recovered per its
  retry policy and the pool replaced. The old wording read as task loss
  when no task is lost.

### Fixed

- The agent skill's task-info example steered readers to the broker-level
  `get_task_info_async`, whose `decoded_result` is always empty; it now
  shows `app.get_task_info_async` (typed decode) as the primary path.
...
- Schema v16: indexes for the monitoring read path — `enqueued_at` (task
  list default sort) and `task_name` (facet index-only scan). Neither
  column appears in any worker-path predicate or leading sort key; a plan
  test pins the claim statement to `idx_horsies_tasks_claim_pending`.

### Changed

- The monitoring task list loads only the columns the list view renders;
  `args`, `kwargs`, `result`, and `task_options` are no longer fetched,
  so a list page stops shipping task payloads (previously up to several
  MB of egress per 50-row page) and stops detoasting large results.
- The list's `total` is a planner estimate (`pg_class.reltuples`) on the
  unfiltered view; any active filter keeps the exact count. A
  never-sampled table falls back to the exact count.
- Sort ordering wraps only nullable sort keys in `NULLS LAST`; on the
  NOT NULL `enqueued_at` the wrapper blocked index-order scans while
  changing no results.
- Web UI: the task aggregates (stats, facets, breakdown) no longer
  refetch on every task event. They refresh on their existing cadences
...
- The docs changelog frontmatter failed YAML parsing (a `: ` sequence in
  the 0.4.2 summary); the docs site had not deployed since the v0.4.2
  merge.
...
- `horsies.monitoring` (core, no extra): typed read-query API over tasks,
  workflows, workers, and schedules, with `error_category` filtering expanded
...
- `cancel_task` / `retry_task`: registry-free, single-transaction
  ...
  claim, finalize, auto-retry, or reaper paths. Retry reuses the task row,
  preserves attempt history, never modifies `max_retries` or `good_until`,
  and emits the queue NOTIFY the insert trigger does not fire for updates.
  Cancelling a RUNNING task requires `include_running=True`: the row flips
  durably but the process keeps executing. Workflow-bound rows are refused.
- `horsies[web]` extra: `create_monitoring_app(app, *, auth_policy, config)`
  and the `horsies web` CLI (app-path form, or registry-less
  `--database-url` form — the latter supports all reads, task actions, and
  workflow pause/cancel, but not resuming runs whose next nodes carry
  `args_from`). The dashboard SPA ships inside the wheel; no Node at runtime.
  The library never owns identity: mounted mode requires an explicit
  `MonitoringAuthPolicy`; the CLI is fail-closed (loopback default,
  trusted-header auth required off-loopback, actions off unless
  `--enable-actions`).
- No-DDL guarantee: brokers constructed by the web layer skip schema
  migrations; schema state is probed and reported as MATCH / MISMATCH /
  ABSENT / UNKNOWN. Mismatch serves reads and refuses actions; absence is
  reported, never initialized; an unreachable database is UNKNOWN, never
  conflated with absence.
- Live updates over SSE from the existing monitoring NOTIFY channels
  (one listener per web process, coalesced invalidation events); client
  polling is a fallback only.
- `showcase/`: Acme Clothing, a runnable demonstration application — 35
  tasks, 12 workflow definitions, 31 schedules, 8 scenarios — with
  deterministic failure draws. See the Quick Start showcase page.
- CI: `webui` and `showcase` jobs; the release workflow blocks unless the
  wheel carries the built dashboard assets.

### Fixed

- `args_from` envelopes now decode for optional targets: the
  `TaskResult[T, TaskError] | None` annotation — the documented requirement
  for `join='any'`/`'quorum'` targets — previously failed
  `WORKER_SERIALIZATION_ERROR` whenever the source resolved in time. Unions
  are unwrapped before the envelope passthrough test.
