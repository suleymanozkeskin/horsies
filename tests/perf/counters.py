...
# WAL is attributed from top-level statements only: pg_stat_statements reports
# the WAL generated by a function call on both its top-level SELECT and the
# nested statements that produced it. Summing both layers makes an otherwise
# identical PL/pgSQL wrapper appear to write the same WAL twice. Nested calls
# and rows stay visible as diagnostics, but they are not a second WAL event.
_READ_SQL = text("""
    SELECT
...
class Counts:
    ...
    def per_row(self, total: int) -> float:
        return total / self.terminal_rows if self.terminal_rows else 0.0
...
class CounterProbe:
    ...
    def finish(self, *, terminal_rows: int) -> Counts:
    ...
    def _read_write_transactions(self) -> int:
        return int(self._connection.execute(_WRITE_TRANSACTIONS_SQL).scalar_one())
